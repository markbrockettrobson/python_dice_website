...
class TestAtMostAPI(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._mock_python_dice_interpreter.get_at_most_histogram.return_value = (
    ...
    def test_post(self):
        ...
        self._mock_python_dice_interpreter.get_at_most_histogram.assert_called_once_with(
            ["1 + 3"]
        )
    ...
    def test_get(self):
        response = self._test_app.get("/api/at_most?program=2%20%2B%203")
        self._mock_python_dice_interpreter_factory.get_interpreter.assert_called_once()
        self._mock_python_dice_interpreter.get_at_most_histogram.assert_called_once_with(
            ["2 + 3"]
        )
    ...
    def test_error_post(self):
        self._mock_python_dice_interpreter.get_at_most_histogram.side_effect = ValueError(
            "mock_error"
        )
        response = self._test_app.post(
            "/api/at_most",
    ...
    def test_error_get(self):
        ...
        response = self._test_app.get("/api/at_most?program=ABS%281%20%2D%203d30")
    ...
    def test_post_no_program(self):
        response = self._test_app.post(
            "/api/at_most", data=flask.json.dumps({}), content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
    ...
    def test_get_no_program(self):
        response = self._test_app.get("/api/at_most")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
