...
class TestAverageAPI(unittest.TestCase):
    def setUp(self) -> None:
        self._mock_average = 12345

        self._mock_python_dice_interpreter = mock.create_autospec(
            spec=python_dice.PythonDiceInterpreter, spec_set=True
        )
        self._mock_python_dice_interpreter.get_average.return_value = {
            "stdout": self._mock_average
        }
    ...
    def test_post(self):
        ...
        self._mock_python_dice_interpreter.get_average.assert_called_once_with(
            ["1d2 + 3"]
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_data(as_text=True), f"{self._mock_average}\n")
    ...
    def test_error_post(self):
        ...
        self._mock_python_dice_interpreter.get_average.assert_not_called()
    ...
    def test_post_no_program(self):
        response = self._test_app.post(
            "/api/average", data=flask.json.dumps({}), content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
    ...
    def test_get_no_program(self):
        response = self._test_app.get("/api/average")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
