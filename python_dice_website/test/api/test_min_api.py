...
class TestMinAPI(unittest.TestCase):
    ...
    def test_post(self):
        ...
        self._mock_python_dice_interpreter.min.assert_called_once_with(["1d2 + 3"])
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_data(as_text=True), f"{self._mock_min}\n")

    def test_get(self):
        response = self._test_app.get("/api/min?program=2%20%2B%203d2")
        self._mock_python_dice_interpreter_factory.get_interpreter.assert_called_once()
        self._mock_python_dice_interpreter.min.assert_called_once_with(["2 + 3d2"])
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_data(as_text=True), f"{self._mock_min}\n")

    def test_error_post(self):
        self._mock_python_dice_interpreter.min.side_effect = ValueError("mock_error")
        response = self._test_app.post(
            "/api/min",
        ...
        self._mock_python_dice_interpreter_factory.get_interpreter.assert_called_once()
        self._mock_python_dice_interpreter.min.assert_called_once_with(["ABS(1 - 3d30"])
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.get_data(as_text=True), '"mock_error"\n')

    def test_error_get(self):
        self._mock_python_dice_interpreter.min.side_effect = ValueError("mock_error")
        response = self._test_app.get("/api/min?program=ABS%281%20%2D%203d30")
    ...
    def test_post_no_program(self):
        response = self._test_app.post(
            "/api/min", data=flask.json.dumps({}), content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
    ...
    def test_get_no_program(self):
        response = self._test_app.get("/api/min")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
