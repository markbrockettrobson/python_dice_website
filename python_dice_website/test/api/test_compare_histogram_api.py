...
class TestCompareHistogramAPI(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._mock_distribution_1.get_compare_histogram.return_value = (
    ...
    def test_post(self):
        ...
        self._mock_distribution_1.get_compare_histogram.assert_called_once_with(
            self._mock_distribution_2
        )
    ...
    def test_get_no_programs(self):
        response = self._test_app.get("/api/compare_histogram")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
    ...
    def test_get_no_program_one(self):
        response = self._test_app.get("/api/compare_histogram?program_two=3d4")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.get_data(as_text=True), f'"No url parameter program_one."\n'
        )
    ...
    def test_get_no_program_two(self):
        response = self._test_app.get("/api/compare_histogram?program_one=2d6")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
