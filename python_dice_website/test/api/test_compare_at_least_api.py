...
class TestCompareAtLeastAPI(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._mock_distribution_1.get_compare_at_least.return_value = (
    ...
    def test_post(self):
        ...
        self._mock_distribution_1.get_compare_at_least.assert_called_once_with(
    ...
    def test_get_no_name_one(self):
        response = self._test_app.get(
            "/api/compare_at_least?program_one=2d6&program_two=3d4&program_two_name=Name%20two"
        )
        self._mock_python_dice_interpreter_factory.get_interpreter.assert_called_once()
        self._mock_python_dice_interpreter.get_probability_distributions.assert_has_calls(
            [mock.call(["2d6"]), mock.call(["3d4"])]
    ...
    def test_get_name_two(self):
        response = self._test_app.get(
            "/api/compare_at_least?program_one=2d6&program_two=3d4&program_one_name=Name%20one"
        )
        self._mock_python_dice_interpreter_factory.get_interpreter.assert_called_once()
        self._mock_python_dice_interpreter.get_probability_distributions.assert_has_calls(
            [mock.call(["2d6"]), mock.call(["3d4"])]
    ...
    def test_get_no_programs(self):
        response = self._test_app.get("/api/compare_at_least")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
    ...
    def test_get_no_program_one(self):
        response = self._test_app.get("/api/compare_at_least?program_two=3d4")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.get_data(as_text=True), f'"No url parameter program_one."\n'
        )
    ...
    def test_get_no_program_two(self):
        response = self._test_app.get("/api/compare_at_least?program_one=2d6")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
